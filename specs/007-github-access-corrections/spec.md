...
- **FR-087**: The standing prohibition on printing the credential MUST be restored verbatim, and no
  ...
  this change governs. An operator-facing command a human runs by hand is a different thing and is
...
- **SC-008**: Neither file this change governs — `CLAUDE.md` and
  [docs/github-access.md](../../docs/github-access.md) — contains a route by which an agent
  obtains the credential for its **own** use.

  Scoped deliberately, because the unscoped version would be false on the day it merged. One
  extraction recipe remains in `CLAUDE.md`: the `--set-key` / branch-protection call an operator
  runs by hand. It predates this change, it is addressed to a human rather than to an agent, and
