...
- **FR-076**: Each required reviewer role MUST resolve to an ordered list of providers — its
  *route* — declared in the operating settings and reported as effective with each run (FR-054). A
  ...
  than can be true.
- **FR-077**: At least one provider outside the model family used today MUST be supported, behind
  the existing `ModelClient` boundary (FR-029) and with no second seam. One prompt builder and one
...
- **SC-006**: Adding a further provider later requires no change to the reviewer roles, the gate, or
  the prompt and parser — only a new provider declaration and its containment statement.

- **SC-007**: No required reviewer role runs on the provider that authored the diff, and an attempt
  to configure one fails before any model call rather than after a review has been paid for.
