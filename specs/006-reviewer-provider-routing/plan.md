...
> **Scope corrected 2026-09-20.** A provider outside the Claude family is no longer in scope, so
> the third pull request this plan describes — the Antigravity adapter — is not built. The
> packaging is two parts, not three. R-020's findings about driving `agy` headless were verified
> and remain accurate; they are kept because they are reusable the day a second family is wanted,
> not because anything here ships them. Where this document says "the first provider", read it as
> describing the plan as drawn rather than the work as delivered.
...
**Two pull requests, not the three an earlier revision of this section described.**

| PR | Content |
|---|---|
| **PR 1a** | Types, settings shape, migration, every invariant that validates a declaration, the routing composite, failure classification, scripted e2e |
| **PR 1b** | Per-provider credential preflight, shared-account reporting, per-account budgets, per-provider attribution in the durable record |

The third — the Antigravity adapter, its containment declaration, its credential preflight and the
R-027 quality replay — is **not built**. A provider outside the Claude family went out of scope on
2026-09-20 (spec.md, Clarifications), and this section is corrected rather than left asserting a
plan of record that nobody is executing. Two artifacts created in the same change must not arrive
contradicting each other.

What the remaining two deliver is availability rather than independence: `api` and `agent-sdk` are
one model family on two funding accounts, which is R-029's distinction and the property pull
request #9 actually lost.
