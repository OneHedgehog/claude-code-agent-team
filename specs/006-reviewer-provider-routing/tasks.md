...
**FR-077 and SC-007 are corrected in `spec.md` rather than contradicted from here.** The spec now
requires more than one provider across more than one funding account — what is actually built —
and records a second model *family* as out of scope. SC-007 is stated as not met and not claimed.
FR-082's check ships built, tested and dormant, because declaring an `authoringProvider` when
every reachable provider is the authoring family would refuse every configuration there is.

An earlier revision of this file declared the scope change here and left the spec asserting a MUST
it would not meet. That was the wrong artifact: Principle IX protects a *merged* spec as the
record of intent, and this one has never merged.
